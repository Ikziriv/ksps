...
class UserController extends Controller 
{
	...
	private function indexGo($role, $ajax = false)
	{
		...
		$users = $this->user_gestion->index(7, $role); 
		...
		if($ajax)
		{
			return response()->json([
				'view' => view('admin.pages.users.table', compact('users', 'links', 'counts', 'roles'))->render(), 
				'links' => str_replace('/sort/total', '', $links)
			]);			
		}

		return view('admin.pages.users.index', compact('users', 'links', 'counts', 'roles'));		
	}
	/**
	...
	public function getRoles()
	{
		...
		return view('admin.pages.users.roles', compact('roles'));
	}
